...
from .p2p_inference_cell_updater import _P2PInferenceCellUpdater
    ...
    def connect(
        ...
        if planned_targets:
            ...
            for engine_ind, error in query.failures_by_engine_ind.items():
                updaters_by_cell_id[engine_cell_ids[engine_ind]].mark_errored(error)

            for target in planned_targets:
                if updaters_by_cell_id[engine_cell_ids[target.engine_ind]].is_errored:
                    continue
        ...
        for engine_rank, rank_targets in targets_grouped_by_engine_rank.items():
            ...
            self._transfer_engine_meta_list.append(
                _TransferEngineMeta(
                    ...
                    cell_updaters=[updaters_by_cell_id[engine_cell_ids[target.engine_ind]] for target in rank_targets],
                )
            )
